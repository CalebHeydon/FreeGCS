...
int System::solve_BFGS(SubSystem *subsys, bool /*isFine*/, bool isRedundantsolving)
{
    ...
    if(debugMode==IterationLevel) {
        std::cout   << "BFGS: convergence: "    << (isRedundantsolving?convergenceRedundant:convergence)
                    << ", xsize: "              << xsize
    ...
    for (int iter=1; iter < maxIterNumber; iter++) {
        ...
        if (err > divergingLim || err != err) { // check for diverging and NaN
            if(debugMode==IterationLevel) {
                std::cout   << "BFGS Failed: Diverging!!: "
                            << ", err: "              << err
...
SolverReportingManager::Manager().LogToFile("gcs::System::diagnose()\n");
    ...
    if(qrAlgorithm==EigenSparseQR){
        std::cout << "SparseQR not supported by you current version of Eigen. It requires Eigen 3.2.2 or higher. Falling back to Dense QR\n";
    ...
    if(qrAlgorithm==EigenDenseQR){
        ...
        std::cout << "\nDenseQR - Lapsed Time: " << SolveTime  << " seconds\n";
    #endif
    }
        ...
        std::cout << "\nSparseQR - Lapsed Time: " << SolveTime  << " seconds\n";
        #endif
    }
#endif
                                                    ...
                                                    )
{
    ...
    if(debugMode==Minimal || debugMode==IterationLevel) {
        ...
        std::cout << "RedundantSolving-" << solvername << "-\n";
    }
    ...
    if (res == Success) {
        ...
        if(debugMode==Minimal || debugMode==IterationLevel) {
            std::cout << "Redundant solving: " << redundant.size() << " redundants\n";
        }
        ...
        for (int i=conflictGroupsOrig.size()-1; i >= 0; i--) {
            ...
            for (std::size_t j=0; j < conflictGroupsOrig[i].size(); j++) {
                if (redundant.count(conflictGroupsOrig[i][j]) > 0) {
                    ...
                    if(debugMode==IterationLevel) {
                        std::cout << "(Partially) Redundant, Group " << i << ", index " << j << ", " << j << ", Tag: " << conflictGroupsOrig[i][j]->getTag() << "\n";
                    }
